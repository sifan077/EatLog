    ...
    <DashboardLayout>
      <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-teal-50 to-emerald-50 pt-4">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">{today}</h2>
          <p className="text-sm sm:text-base text-gray-600 mt-1">{displayName}</p>
        </div>
        ...
        <main className="max-w-4xl mx-auto px-4 pb-8 sm:px-6 sm:pb-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-900">今日记录</h2>
              <span className="text-sm text-gray-600">{mealLogsWithUrls.length} 条记录</span>
            </div>

            {mealLogsWithUrls.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-8 sm:p-12 text-center border border-white/50">
                <div className="text-6xl mb-4">🍽️</div>
                <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">还没有记录</h3>
                <p className="text-gray-600 text-sm sm:text-base">开始记录你的第一餐吧！</p>
              </div>
            ) : (
